...
import { ShopLayout } from '../../shop/components';
import { colors, textShadow } from '../../../styles/theme';

// Inline SVG Icon Components
const HandIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="1.3021vw" height="2.3148vh" viewBox="0 0 25 25" fill="none">
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{
        ...
        >
          {segmentPaths.map((path, i) => <path key={i} d={path} fill={colors.primary.main} fillOpacity={1} />)}
        </svg>
      </Box>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        ...
        style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)' }}
      >
        {segmentPaths.map((path, i) => <path key={i} d={path} fill={colors.primary.main} fillOpacity={0.05} />)}
      </svg>
    </Box>
  );
};
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1.2vh' }}>
            ...
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', gap: '.5208vw', alignItems: 'center' }}>
                <Box sx={{
                  width: '2.0833vw',
                  height: '3.7037vh',
                  borderRadius: '.2083vw',
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', gap: '.5208vw', alignItems: 'center' }}>
                  ...
                  <Box>
                    ...
                    <Typography sx={{ color: colors.text.secondary, fontSize: '.625vw', fontWeight: 400, fontFamily: 'Rubik, sans-serif' }}>
                      Primary inventory space
                    </Typography>
                  </Box>
                </Box>
                {/* Right: Weight Display */}
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '.3vh' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: '.5208vw' }}>
                    <Typography sx={{ color: colors.text.primary, textShadow, fontSize: '.625vw', fontWeight: 600, letterSpacing: '0.6px', fontFamily: 'Rubik, sans-serif' }}>
                      {playerWeight.toFixed(1)} / {player.capacity} lbs
                    </Typography>
                    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '1.3021vw', height: '2.3148vh', borderRadius: '.1563vw', background: colors.primary.alpha(0.15) }}>
                      <WeightIcon />
                    </Box>
                  </Box>
                  <SegmentedWeightBar currentWeight={playerWeight} maxWeight={player.capacity} />
                </Box>
              </Box>
              ...
              <Box sx={{
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '.3vh' }}>
                    ...
                    <SegmentedWeightBar currentWeight={secondaryWeight} maxWeight={secondary.capacity} />
                  </Box>
                </Box>

                {/* Secondary Inventory Slots */}
                <Box
                  sx={{
                    display: 'grid',
                    gridTemplateColumns: 'repeat(5, 1fr)',
